 ...
 * className — дополнительные классы картинки (например, скругление rounded-md);
 * captionClass — классы подписи вместо text-label (например, text-[13px]).
 */
    ...
    <figure className="m-0">
      {resolved ? (
        ...
        <div
          ...
          className={`flex h-[120px] w-full items-center justify-center border ${border} text-label ${muted} ${className}`}
        >
          фото
        </div>
      )}
      {caption && <figcaption className={`mt-2 leading-[1.2] ${captionClass} ${muted}`}>{caption}</figcaption>}
    </figure>
  )
}
